...
" Format tag
syn match sasFormatTag "\v\w+\.\W@=" display contained
...
" Functions
syn region sasFunction matchgroup=sasFuncName start="\v<%(call\s+)=\w+\(@=" end="\v\)@=" contains=@sasBasicSyntax
syn region sasMacroFunc matchgroup=sasMacroFuncName start="\v\%\w+\(@=" end="\v\)@=" contains=@sasBasicSyntax
...
syn match sasProcSQLStatement "\v%(^|;)@<=\s*%(alter( table)=|connect|create%( %(index|table|view))=|delete|describe%( %(table|view))=|disconnect|drop%( %(index|table|view))=|execute|insert|reset|select|update|validate)>" display contained
syn region sasProcSQL start="\v%(^|;)@<=\s*proc\s+sql>" end="\v%(^|;)@<=\s*((run|quit)>|((data|proc|endsas)>)@=)" fold contains=@sasBasicSyntax,sasGlobalStatement,sasProcSQLClause,sasProcSQLStatement
...
syn match sasDS2Statement "\v%(^|;)@<=\s*%(array|by|%(dcl|declare|drop)%( %(package|thread))=|forward|keep|merge|output|put|rename|retain|set%( from)=|stop|vararray|varlist)>" display contained
syn region sasDS2 start="\v%(^|;)@<=\s*proc\s+ds2>" end="\v%(^|;)@<=\s*((run|quit)>|((data|proc|endsas)>)@=)" fold contains=@sasBasicSyntax,sasGlobalStatement,sasDS2Control,sasDS2Statement

" SAS/IML, 14.1
syn keyword sasIMLControl by data do else end goto if link return then to until while contained
syn match sasIMLStatement "\v%(^|;)@<=\s*%(abort|append%( %(var|from))=|call|close|closefile|create|delete|display|file|find|finish|free|index%( none)=|infile|input|list|load|mattrib|pause|print|purge|put|quit|read|remove|replace|reset|resume|remove|run|save|setin|setout|show|sort|start|stop|store|summary|use|window)>" display contained
syn region sasIML start="\v%(^|;)@<=\s*proc\s+iml>" end="\v%(^|;)@<=\s*((quit)>|((data|proc|endsas)>)@=)" fold contains=@sasBasicSyntax,sasGlobalStatement,sasIMLControl,sasIMLStatement
