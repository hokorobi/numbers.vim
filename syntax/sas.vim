...
syn cluster sasBasicSyntax contains=sasString,sasNumber,sasComment,sasFunction,sasOperator,sasReserved,sasMacroReserved,sasMacroFunc,sasMacroVariable,sasSectLbl
...
syn region sasProcTemplate start="\v%(^|;)@<=\s*proc\s+template>" end="\v%(^|;)@<=%(\s*%(data|endsas|proc|quit|run)>)@=" contains=@sasBasicSyntax,sasFormat,sasGlobalStatement,sasProcTemplateStatement,sasGTLStatement
...
syn region sasProcSQL start="\v%(^|;)@<=\s*proc\s+sql>" end="\v%(^|;)@<=%(\s*%(data|endsas|proc|quit|run)>)@=" contains=@sasBasicSyntax,sasFormat,sasGlobalStatement,sasProcSQLClause,sasProcSQLStatement
...
syn region sasDS2 start="\v%(^|;)@<=\s*proc\s+ds2>" end="\v%(^|;)@<=%(\s*%(data|endsas|proc|quit|run)>)@=" contains=@sasBasicSyntax,sasFormat,sasGlobalStatement,sasDS2Control,sasDS2Statement
...
syn region sasIML start="\v%(^|;)@<=\s*proc\s+iml>" end="\v%(^|;)@<=%(\s*%(data|endsas|proc|quit)>)@=" contains=@sasBasicSyntax,sasFormat,sasGlobalStatement,sasIMLControl,sasIMLStatement

" SAS format statement
syn match sasFormatTag "\v\w+\." display contained
...
" Macro definition
syn region sasMacro start="\v\%macro>" end="\v\%mend>" contains=@sasBasicSyntax,sasGlobalStatement,sasDataStepControl,sasDataStepStatement,sasFormat,sasDataStep,sasProc,sasGraphProc,sasAnalyticalProc,sasProcTemplate,sasProcSQL,sasDS2,sasIML
...
hi def link sasFormatStatement Statement
